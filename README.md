...
6. How to determine which IP address AWS CloudFormation uses to deploy MongoDB Atlas resouces with my Atlas Programmatic API Keys?
  When you deploy MongoDB Atlas using CloudFormation with your Atlas Programmatic API Keys, CloudFormation will use the IP address of the machine from which you are making the API call. Therefore, you should ensure that the machine you are making the API call from has a fixed IP address or a range of IP addresses that are whitelisted in your MongoDB Atlas project. To check the IP address of the machine you are making the API call from, you can visit http://checkip.amazonaws.com. Once you have your IP address, you can add it to your MongoDB Atlas project's IP whitelist.
