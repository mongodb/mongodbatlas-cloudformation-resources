...
do
  ...
  do
    ...
    type_info=$(aws cloudformation --region "${region}"  list-types --output=json | jq --arg typeName "${type_name}" '.TypeSummaries[] | select(.TypeName==$typeName)')
    if [ -z "${type_info}" ]; then
      echo "***********No type found for region : ${region}"
    else
