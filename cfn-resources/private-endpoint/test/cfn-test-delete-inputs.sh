...
if [[ "$PRIVATE_ENDPOINTS" == "[]" ]]; then
  ...
  for ATLAS_PRIVARE_ENDPOINT_SERVICE in $(echo "$PRIVATE_ENDPOINTS" | jq -r '.[].id'); do
    # Delete the VPC endpoint
    echo "STEP 1.a DELETING INTERFACES FOR SERVICE $ATLAS_PRIVARE_ENDPOINT_SERVICE"
    ENDPOINT_OUTPUT=$(atlas privateEndpoints aws describe $ATLAS_PRIVARE_ENDPOINT_SERVICE --projectId $projectId --output json)

    interfaceEndpoints=$(echo "$ENDPOINT_OUTPUT" | jq -r '.interfaceEndpoints[]')

    if [ -z "$interfaceEndpoints" ]; then
        echo "No interfaceEndpoints found for $ATLAS_PRIVARE_ENDPOINT_SERVICE"
        echo "STEP 1.d deleting privateEndpoint Service "
        atlas privateEndpoints aws delete $ATLAS_PRIVARE_ENDPOINT_SERVICE --projectId $projectId --force
    else
      for interfaceId in $(echo "$ENDPOINT_OUTPUT" | jq -r '.interfaceEndpoints[]'); do
            echo "STEP 1.b DELETING INTERFACE $interfaceId FOR SERVICE $ATLAS_PRIVARE_ENDPOINT_SERVICE"
            atlas privateEndpoints aws interface delete $interfaceId --endpointServiceId $ATLAS_PRIVARE_ENDPOINT_SERVICE --projectId $projectId --force

            sleep 20 #waiting until the connection gets rejected
            echo "STEP 1.c DELETING aws private endpoint $interfaceId FOR SERVICE $ATLAS_PRIVARE_ENDPOINT_SERVICE"
