...
var (
	CreateRequiredFields = []string{constants.ProjectID, constants.ClientID}
	ReadRequiredFields   = []string{constants.ProjectID, constants.ClientID, constants.SecretID}
	DeleteRequiredFields = []string{constants.ProjectID, constants.ClientID, constants.SecretID}
	ListRequiredFields   = []string{constants.ProjectID, constants.ClientID}
)

func setupRequest(req handler.Request, model *Model, requiredFields []string) (*util.MongoDBClient, *handler.ProgressEvent) {
	util.SetupLogger("mongodb-atlas-project-service-account-secret")
