...
func FailIfResourceEnvNotSet(t *testing.T, resourceName string) {
	t.Helper()

	if os.Getenv("RESOURCE_TYPE_NAME") == "" {
		FailNowWithMsg(t, fmt.Sprintf("Required environment variables not set for %s resource.", resourceName))
	}
}
...
func FailNowWithMsg(t *testing.T, msg string) {
	t.Error(msg)
	t.FailNow()
}

//func RunCleanupScript(t *testing.T, rctx ResourceContext) {
//	output, err := runShScript("../cleanup_cfn.sh")
...
//
//	output, err := runShScript("../publish_cfn_to_registry.sh")
